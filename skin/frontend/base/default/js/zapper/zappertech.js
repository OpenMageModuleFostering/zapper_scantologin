...
var ZapperTech = (function($) {
    var ZapperTech = ZapperTech || {};

    ZapperTech.QrCode = function(options, callback) {
        this.context = {};
        this.defaults = {
            baseZapperTechUrl: 'http://zl.qr.ai?info={taskId}:{merchantId}:{siteId}:{sessionId}:{encryptionKey}:{questions}',
    ...
    //Settings: merchantId, siteId, taskId, qrSize, selector
    ZapperTech.QrCode.prototype.init = function(settings, callback) {
        settings = $.extend({}, this.defaults, settings);

        settings.basePollUrl = settings.baseUrl + "/api/clientsession";
        settings.baseQuestionsUrl = settings.baseUrl + "/api/merchants/{merchantId}/sites/{siteId}/questions/{taskId}/embed?callback=?";
        settings.qrCodeImageUrl = settings.baseUrl + "/api/merchants/{merchantId}/sites/{siteId}/tasks/{taskId}/generatecode?size={size}&sessionid={sessionId}&encryptionkey={encryptionKey}&brushColor={brushColor}&backgroundColor={backgroundColor}&amount={amount}";
        settings.questionsUrl = settings.baseUrl + "/api/questionsembed?callback=?";

        var sessionId = generateSessionId();
        var encryptionKey = generateSessionId();

        var imageUrl = settings.qrCodeImageUrl
            .replace('{merchantId}', settings.merchantId)
            .replace('{siteId}', settings.siteId)
            .replace('{taskId}', settings.taskId)
            .replace('{sessionId}', sessionId)
            .replace('{encryptionKey}', encryptionKey)
            .replace('{brushColor}', settings.brushColor)
            .replace('{backgroundColor}', settings.backgroundColor)
            .replace('{size}', settings.qrSize);

        if (settings.taskId == 1 && settings.additionalParameters.length > 0) {
            imageUrl = imageUrl.replace('{amount}', settings.additionalParameters[0]);
        } else {
            imageUrl = imageUrl.replace('{amount}', '0.00');
        }
        ...
        $(settings.selector).append(image);
        ...
        context.basePollUrl = settings.basePollUrl;
        context.pollUrl = settings.basePollUrl + "/" + sessionId + '/';
        context.timeout = settings.timeout;
        context.encryptionKey = encryptionKey;
        context.completeCallback = function() { };
        context.registrationRequest = function() { };
        context.loginRequest = function() { };
        context.paymentRequest = function () { };
        
        //
        // Pull Questions
        //
        $.getJSON(settings.questionsUrl, function (data, status) {
            if (status == "success") {
                context.QUESTIONS = data;
            }
        });

        if (!context.pollRequestTypeSet)
            context.pollRequestTypeSet = new Array();
    };

    ZapperTech.QrCode.prototype.scanComplete = function(requestTypeSet, callback) {
        $(requestTypeSet).each(function(i, requestType) {
            this.context.pollRequestTypeSet.push(requestType);
        });
        this.context.completeCallback = callback || function() {
        };
    };

    ZapperTech.QrCode.prototype.registrationRequest = function(callback) {
        this.context.pollRequestTypeSet.push(this.CONSTANTS.REQUESTTYPE.registrationRequest);
        this.context.registrationRequest = callback || function() {
        };
    };

    ZapperTech.QrCode.prototype.loginRequest = function(callback) {
        this.context.pollRequestTypeSet.push(this.CONSTANTS.REQUESTTYPE.loginRequest);
        this.context.loginRequest = callback || function() {
        };
    };

    ZapperTech.QrCode.prototype.paymentRequest = function(callback) {
        this.context.pollRequestTypeSet.push(this.CONSTANTS.REQUESTTYPE.paymentRequest);
        this.context.paymentRequest = callback || function() {
        };
    };

    ZapperTech.QrCode.prototype.poll = function() {
        if (this.context.pollRequestTypeSet.length === 0)
            this.context.pollRequestTypeSet.push(this.CONSTANTS.REQUESTTYPE.registrationRequest);
        var pollUrl = this.context.basePollUrl + '/' + this.context.sessionId + '/' + this.context.pollRequestTypeSet.join(",") + '?callback=?';
        var code = this;
        $.getJSON(pollUrl, function(data, status) {
            code.pollHandle(data, status);
        });
    };

    ZapperTech.QrCode.prototype.getAnswer = function (answers, questionId) {
        var code = this;
        var resultingAnswer = null;
        $(answers).each(function(i, answer) {
            if (answer.QuestionId !== undefined && answer.QuestionId === questionId && answer.QuestionId !== code.QUESTIONTYPE.custom)
                resultingAnswer = mapPickerValues(answer, code.context.QUESTIONS);
            else if (answer.QuestionId === code.QUESTIONTYPE.custom && answer.CustomQuestionText === questionId)
                resultingAnswer = answer.AnswerValue;
        });
        return resultingAnswer;
    };

    ZapperTech.QrCode.prototype.getPickerId = function (answerObject) {
        var code = this;
        return mapPickerValues(answerObject, code.context.QUESTIONS);
    };

    ZapperTech.QrCode.prototype.sessionId = function() {
        return this.context.sessionId;
    };

    ZapperTech.QrCode.prototype.pollHandle = function(responseSet, status) {
        var code = this;
        if (status === "success") {
            $(responseSet).each(function(i, response) {
                if (response.Data && response.Data !== null) {
                    //var decryptedData = JSON.parse(response.Data);
                    var decryptedData = JSON.parse(decrypt(response.Data, code.context.encryptionKey));
                    code.context.completeCallback(response.RequestType, decryptedData);
                    if (response.RequestType === code.CONSTANTS.REQUESTTYPE.registrationRequest) {
                        decryptedData.getAnswer = function(questionId) {
                            return code.getAnswer(decryptedData.Answers, questionId);
                        };
                        code.context.registrationRequest(decryptedData);
                    }
                    if (response.RequestType === code.CONSTANTS.REQUESTTYPE.loginRequest)
                        code.context.loginRequest(decryptedData);

                    if (response.RequestType === code.CONSTANTS.REQUESTTYPE.paymentRequest)
                        code.context.paymentRequest(decryptedData);
                }
            });
        }

        this.context.timer = window.setTimeout(function() { code.poll(); }, this.context.timeout);
    };

    ZapperTech.QrCode.prototype.respond = function(type, data, afterRespondHandler) {
        afterRespondHandler = afterRespondHandler || function() {
        };
        var respondData = {
            RequestType: type,
            //Data: JSON.stringify(data), 
            Data: encrypt(JSON.stringify(data), this.context.encryptionKey),
            SessionId: this.context.sessionId
        };
        $.ajax({
            url: this.context.basePollUrl,
            data: respondData,
            dataType: 'jsonp',
            type: 'post',
            complete: function(response, success) {
                afterRespondHandler(response);
            }
        });
    };

    ZapperTech.QrCode.prototype.registrationRespond = function(data, afterRespondHandler) {
        this.respond(this.CONSTANTS.REQUESTTYPE.registrationResponse, data, afterRespondHandler);
    };

    ZapperTech.QrCode.prototype.loginRespond = function(data, afterRespondHandler) {
        this.respond(this.CONSTANTS.REQUESTTYPE.loginResponse, data, afterRespondHandler);
    };

    ZapperTech.QrCode.prototype.start = function() {
        this.poll();
    };

    ZapperTech.QrCode.prototype.stop = function() {
        if (this.context.timer !== undefined)
    ...
    var decrypt = function(cypherText, encryptionKey) {
        try {
            var ivHex = CryptoJS.enc.Hex.parse(encryptionKey.toString().split("").reverse().join(""));
            var keyHex = CryptoJS.enc.Hex.parse(encryptionKey);
            var dec128 = CryptoJS.AES.decrypt(cypherText, keyHex, { iv: ivHex });
            if ($.trim(dec128).length === 0) {
                throw ("AES-128 Failed");
            }
            return dec128.toString(CryptoJS.enc.Utf8);
        } catch(err) {
            var dec = CryptoJS.AES.decrypt(cypherText, encryptionKey);
            return JSON.parse((dec).toString(CryptoJS.enc.Utf8));
        }
    };

    var encrypt = function(plainText, encryptionKey) {
        try {
            var ivHex = CryptoJS.enc.Hex.parse(encryptionKey.toString().split("").reverse().join(""));
            var keyHex = CryptoJS.enc.Hex.parse(encryptionKey);
            var cypherText = CryptoJS.AES.encrypt(plainText, keyHex, { iv: ivHex });
            if ($.trim(cypherText).length === 0) {
                throw ("AES-128 Failed");
            }
            return cypherText.toString();
        } catch(err) {
            var cypherText = CryptoJS.AES.encrypt(plainText, encryptionKey);
            return cypherText.toString();
        }
    };

    function mapPickerValues(answerObject, questions){
        var returnValue  = answerObject.AnswerValue;
        var question;
        $(questions.Questions).each(function(i, q){
            if(q.Id == answerObject.QuestionId){
                question = q;
            }
        });

        if(question.QuestionPickerListId){

            returnValue = pickerIdById(answerObject, question, questions.PickerLists);
            if(!returnValue){
                returnValue = pickerIdByPickerValue(answerObject, question, questions.PickerLists) || answerObject.AnswerValue;
            }
    ...
    function pickerIdByPickerValue(answerObject, question, pickerLists) {
        var returnValue;
        $(pickerLists).each(function(i, p){
            if(p.Id == question.QuestionPickerListId){
                
                $(p.Values).each(function(index, value){
                    if(value.PickerValue == answerObject.AnswerValue){
                        returnValue = value.PickerId;
                    }
                });
            }
        });
    ...
    function pickerIdById(answerObject, question, pickerLists) {
        var returnValue;
        $(pickerLists).each(function(i, p){
            if(p.Id == question.QuestionPickerListId){
                
                $(p.Values).each(function(index, value){
                    if(value.Id == answerObject.AnswerValue){
                        returnValue = value.PickerId;
                    }
                });
            }
    ...
    function createGroupedQuestionList(questions) {
        var groups = new Array();
        var list = new Array();

        $(questions).each(function(index, question) {
            var groupId = question.GroupId ? question.GroupId : 8;
            question.GroupId = groupId;
            questions[index] = question;

            if (groups.indexOf(question.GroupId) < 0)
                groups.push(groupId);
        });
        ...
        $(groups).each(function(index, groupId) {
            ...
            groupItem.questions = new Array();

            $(questions).each(function() {
                if (this.GroupId == groupId) {
                    groupItem.questions.push(this);
                }
            });
